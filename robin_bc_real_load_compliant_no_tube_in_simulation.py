...
E_Y = 7.0e10  # glass modulus
nu = 0.22  # Value from CSM3, turek-hron
mu = E_Y / (2.0 * (1.0 + nu))
...
T_other = [-161.54457603, -203.91362716, -214.12732662, -254.81050955, -305.6570104, -336.34554574, -340.15703811, -395.64278745, -442.812403, -498.51916676]
panel_panel_gap = 0.03
x_panel_1 = np.arange(10)*(panel_length+panel_panel_gap) + panel_length/2
for i in range(N):
    phi_i = 0
    phi_1_i = 0
    for k in range(i, N):
        phi_i += x_panel[i]/G/Ip*(T_p1[k] + T_p2[k] + T_q1[k] + T_q2[k]) # on undeformed configuration
        phi_1_i += x_panel_1[i]/G/Ip*T_other[k]
    for j in range(i):
        phi_i = phi_i + (T_p1[j] + T_p2[j] + T_q1[j] + T_q2[j])*x_panel[j]/G/Ip
        phi_1_i = phi_1_i + T_other[j]*x_panel_1[j]/G/Ip
    phi.append(phi_i)
    phi_1.append(phi_1_i)
...
# get the dof at the right bottom edge and left bottom edge of block used to calculate the rotation of panel
def right_bottom_line(x):
    return np.logical_and(np.isclose(x[1], block_width/2*np.cos(stow_angle)), np.isclose(x[2], block_width/2*np.sin(stow_angle)))
...
for i in range(N):
    ...
    phi_predict.append(np.mean(np.arcsin((block_width/2*np.sin(stow_angle)+uz_right)/(block_width/2)))-stow_angle)
...
plt.plot(range(1,1+N), np.asarray(phi_1)/2/np.pi*360.0, 'gd', label='other')
plt.grid()
plt.legend()
plt.xlabel('Panel ID')
